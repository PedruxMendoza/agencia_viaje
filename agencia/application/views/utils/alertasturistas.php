	...
	if($msj=='success') { ?>
		<script>
			Swal.fire({
				...
				if (result.value) {
					window.location.href = "../turista_controller/index" ; /*Aqui se cambia por el nombre del controlado que se ira a copiar (../nombre_controller/index)*/
				}
			})
		</script>
	<?php } 
...
<script>
	function alerta_eliminar(dui){
		Swal.fire({
			...
			if (result.value) {
				window.location.href = "../turista_controller/eliminar/"+dui;
			}
		})
	}
