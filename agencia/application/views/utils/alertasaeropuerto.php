	...
	if($msj=='success') { ?>
		<script>
			Swal.fire({
				...
				if (result.value) {
					window.location.href = "../aeropuerto_controller/index" ; /*Aqui se cambia por el nombre del controlado que se ira a copiar (../nombre_controller/index)*/
				}
			})
		</script>
	<?php } 
...
<script>
	function alerta_eliminar(id){
		Swal.fire({
			...
			if (result.value) {
				window.location.href = "../aeropuerto_controller/eliminar/"+id ;
			}
		})
	}
